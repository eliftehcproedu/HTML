...
$(function () {
  ...
  function resetLicenseSections() {
    ...
    $('.license-error').hide();
    $("#activation-email").prop("disabled", false);
        ...
        case 'initialState':
          ...
          function activateLicense() {
            ...
            if (licenseDetails && licenseDetails.email && licenseDetails.onlineLicense) {
              $('#activation-email').val(licenseDetails.email);
          ...
          function validateActivationForm() {
            ...
            const key = $('#activation-key').val();
            if (email && key) {
              $('#activate-validation-error').text('Must provide either email or license key, not both.');
          ...
          $('#activation-email').on("change paste", function() {
            if (!validateActivationForm()) return;
          });

          $('#activation-email').on("keyup", function() {
            const shouldSubmit = $('#activation-submit').data('submit');
            $('#activation-submit').data('submit', false);
            if (shouldSubmit) {
              $('#activation-submit').trigger('click');
            } else {
              if (!validateActivationForm()) return;
            }
          });
          ...
          $('#activation-email').on("keypress", function(e) {
            if ((e.which && e.which === 13) || (e.keyCode && e.keyCode === 13)) {
              if (!$(this).val()) {
                e.preventDefault();
                return false;
              }
              $('#activation-submit').data('submit', true);
            }
          });
          ...
          $('#activation-submit').click(function () {
            ...
            const emailField = $('#activation-email');
            ...
            if (email && !looksLikeEmail.test(email)) {
              $('#activate-validation-error').text('Invalid email address.');
              $('#activate-validation-error').show();
              return;
            }

            if (key && key.length < 50) {
              $('#activate-validation-error').text('Invalid license key.');
              $('#activate-validation-error').show();
              return;
            }
            ...
            $("#activation-email").prop("disabled", true);
